...
## 目的
- この文書は、CIで自動判定する構造検査（依存方向・越境ルール）の配置場所と運用手順を定義する
- 変更時に「どこを更新し、何を実行して整合を確認するか」を迷わない状態を維持する

## 対象範囲
- 対象は Go / TypeScript の構造検査ルールと実行スクリプト
- ドメイン仕様やAPI契約の定義は対象外（`docs/` と `contracts/` で扱う）
