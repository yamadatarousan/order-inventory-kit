...
- [ ] 目的3: Green（実装を追従させる）
  ...
  - [ ] `inventory_reservations` テーブルを追加し、引当/戻しの参照整合を実装する（migration/seed/参照制約）
...
- [ ] 補完目的: 価格決定元の運用主体を商品マスタに統合する（`product_prices` 暫定運用の解消）
  - [ ] 目的1: 仕様を確定する（商品マスタ最小項目と価格決定ルール）
    - [ ] 商品マスタの最小項目を確定する（`sku` / `name` / `status` / `unit_price` / `currency`）
    - [ ] 注文作成時の価格決定ルールを商品マスタ基準で明文化する（販売可能商品のみ受注可、`unit_price` は注文時スナップショット保存）
    - [ ] 既存 `product_prices` の位置づけを「暫定」と明記し、統合/廃止方針を固定する
  - [ ] 目的2: Red（仕様テストを先に作成する）
    - [ ] 不変条件テストに商品マスタ前提の失敗条件を追加する（未存在SKU/販売停止SKUは注文失敗）
    - [ ] 境界一貫性統合テストに商品状態ケースを追加する（販売停止SKUの `POST /orders` は `400`）
    - [ ] リポジトリ契約テストに価格スナップショットの基準を追加する（商品マスタ価格変更後も既存注文明細の `unit_price` は不変）
  - [ ] 目的3: Green（実装を追従させる）
    - [ ] 商品マスタテーブルを追加し、初期データ投入を実装する（migration/seed）
    - [ ] 注文保存時の価格参照元を商品マスタへ切り替える（`order_items.unit_price` 保存）
    - [ ] `product_prices` のデータを商品マスタへ移行し、参照コードを一本化する
    - [ ] 価格決定元切替後のOpenAPI生成物とテスト整合を更新する
  - [ ] 目的4: Refactor/運用で回帰を固定する
    - [ ] `product_prices` 廃止（または互換維持）後の運用手順を明文化する
    - [ ] Refactor後の再実行通過を確認する（追加した商品マスタ関連テストを再実行して通過）

- [ ] 補完目的: 顧客参照整合を `customers` マスタで固定する（`orders.customer_id` の孤立を防ぐ）
  - [ ] 目的1: 仕様を確定する（顧客識別と参照整合ルール）
    - [ ] `customers` の最小項目を確定する（`id` / `status` / `created_at`）
    - [ ] `orders.customer_id` の整合ルールを明文化する（有効顧客IDのみ受理、整合不一致はエラー）
    - [ ] 参照整合方式を固定する（DB FK制約またはアプリケーション参照検証）
  - [ ] 目的2: Red（仕様テストを先に作成する）
    - [ ] 不変条件テストに顧客参照失敗ケースを追加する（未存在customerIdは注文失敗）
    - [ ] 境界一貫性統合テストに顧客状態ケースを追加する（無効customerIdの `POST /orders` は `400`）
    - [ ] リポジトリ契約テストに顧客参照整合を追加する（不整合時に注文が永続化されない）
  - [ ] 目的3: Green（実装を追従させる）
    - [ ] `customers` テーブルを追加し、初期データ投入を実装する（migration/seed）
    - [ ] `orders.customer_id` の参照整合を実装する（FKまたは参照検証）
    - [ ] 注文作成処理を `customers` 参照前提へ切り替える（不整合時は失敗）
    - [ ] 顧客参照整合後のOpenAPI生成物とテスト整合を更新する
  - [ ] 目的4: Refactor/運用で回帰を固定する
    - [ ] 顧客データ運用手順を明文化する（初期投入・無効化・参照整合の扱い）
    - [ ] Refactor後の再実行通過を確認する（追加した顧客参照整合テストを再実行して通過）
...
- [ ] 保留目的: EC運用拡張テーブルの要否を後続フェーズで確定する（現スコープ外だが未管理で放置しない）
  - [ ] 仕様保留: 配送テーブル群（`shipments` / `shipment_items`）の要否と導入時期を確定する
  - [ ] 仕様保留: 住所テーブル（`customer_addresses`）の要否と導入時期を確定する
  - [ ] 仕様保留: 返金/返品テーブル群（`refunds` / `returns`）の要否と導入時期を確定する
