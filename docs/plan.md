...
#### Red確認（タスク1）
- 完了判定の曖昧さ:
  - 本実装レーンと仮実装レーンの成果が混在すると、Phase 6 完了判定が曖昧になる
- 差し替え条件の曖昧さ:
  - 仮実装を本実装へ移行する条件（どのAPIに差し替えるか、いつ削除するか）が未定義だと運用が崩れる
- 用語の曖昧さ:
  - 「実装済み」「仮実装」「着手不可」の語が同じ意味で使われておらず、誤読余地がある

#### 用語定義（タスク1 Refactor）
- 本実装:
  - 実バックエンドAPIに接続し、Phase 6 完了判定に含める実装
- 仮実装:
  - モック（fixture/MSW）で代替し、Phase 6 完了判定に含めない実装
- 差し替え契約:
  - 仮実装ごとに「差し替え先API」「削除条件」「移行先フェーズ」を固定した管理情報

#### 差し替え契約テンプレ（タスク1 Green）
- 必須記載項目（各仮実装画面ごと）:
  - 対象ストーリーID
  - 仮実装対象画面/機能
  - 現在のモック実装（fixture/MSWの参照先）
  - 差し替え先API（HTTPメソッド + パス）
  - 削除条件（何が実装・通過したらモックを削除するか）
  - 移行先フェーズ/タスクID

#### 完了判定ルール（タスク1確定）
- タスク2/3の成果のみを Phase 6 の本実装完了判定に使う
- タスク4/5の成果は Phase 6 の補助成果として扱い、単独では完了扱いにしない
- 仮実装を追加した場合、同時に差し替え契約テンプレの記載を必須とする

#### 実装内容
- [x] タスク1: 2レーン運用と完了判定を固定する（Red: 完了判定が曖昧な状態を確認 / Green: 本実装/仮実装の対象・除外・差し替え条件を明文化 / Refactor: 用語を統一して誤読を防ぐ）
