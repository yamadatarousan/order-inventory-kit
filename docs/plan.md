...
- [x] OpenAPI差分検査のスクリプト雛形
- [x] 生成整合（Go/TSクライアント）のスクリプト雛形
- [x] 構造検査ルール（依存方向/越境）の初期定義
- [x] CIの最小パイプライン（差分→生成→構造→不変条件→境界前提）
...
### Phase 4: 永続化の実装（DBリポジトリ）

#### 目標
インメモリ実装をDB実装に置き換え、永続化を成立させる

#### 実装内容
- [ ] マイグレーションの作成（orders / order_items / payments / inventory）
- [ ] DBリポジトリ実装（注文/決済/在庫）
- [ ] DB接続設定（env/設定ファイル）
