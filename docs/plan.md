...
- [ ] 目的1: 仕様を確定する（契約定義を含む）
  ...
  - [ ] `amount` の意味と単位を固定する（`amount=決済要求額`、最小通貨単位の整数）
  - [ ] 通貨と丸め規則を固定する（単一通貨運用、端数処理なし/ありの規則を明記）
  - [ ] 価格の真実源泉を固定する（クライアント入力価格を信頼せず、サーバ側価格情報で `unit_price` を決定する）
  - [ ] OpenAPI 契約を標準モデルに合わせて更新する（在庫表現と状態遷移の前提を反映）
  - [ ] OpenAPI 契約に価格モデルを反映する（注文入力/応答の金額項目と金額算出前提を追加・更新）
  - [ ] OpenAPI 契約に `POST /payments/confirm` の `409` 分類を追加する（金額不一致/冪等異額再送）
  - [ ] 契約の破壊的変更に対する移行定義を追加する（`contracts/migrations/*.yaml`）

- [ ] 目的2: Red（仕様テストを先に作成する）
  - [ ] 不変条件テストを標準モデルへ更新する（負数禁止、過剰確保失敗、戻し時整合）
  - [ ] 不変条件テストに金額整合を追加する（合計算出、一致時成功、不一致時失敗、冪等異額時失敗）
  - [ ] 境界一貫性統合テストの副作用観測を更新する（orders/payments/inventory の期待値を標準モデルに合わせる）
  - [ ] 境界一貫性統合テストに金額系ケースを追加する（`200/400/404/409` と冪等同額/異額）
  - [ ] `docs/testing-roles.md` の網羅マトリクスと観測項目を標準モデルへ同期する
  - [ ] Red確認ログを残す（追加テストが実装前に失敗していることを作業ログに記録する）
...
- [ ] 目的2: Red（仕様テストを先に作成する）
  ...
  - [ ] Red確認ログを残す（実装前に失敗していることを作業ログに記録する）
